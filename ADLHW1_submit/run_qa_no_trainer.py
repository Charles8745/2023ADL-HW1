...
def main():
    ...
    import pandas as pd
    column_labels = ['epoch', 'steps', 'loss', 'em', 'f1']
    track_df = pd.DataFrame(columns=column_labels)
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if completed_steps % 500 == 0:
                ...
                EM = eval_metric['exact_match']
                F1 = eval_metric['f1']
                print(f'\nsteps: {completed_steps}/{args.max_train_steps}, loss:{loss:.4f}, EM: {EM:.4f}, F1: {F1:.4f}')
                
                # store to df
                new_row = {'epoch':epoch, 'steps':completed_steps, 'loss':loss.item(), 'em':EM, 'f1':F1}
                track_df = pd.concat([track_df, pd.DataFrame([new_row])], ignore_index=True)
    ...
    '''
    I add here [2] output csv file--------------------------------------------------------------------------
    '''
    # output loss csv
    csv_path = os.path.join(args.output_dir, 'lossandEM_track.csv')
    ...
    outputs_numpy = (start_logits_concat, end_logits_concat)
    prediction = post_processing_function(eval_examples, eval_dataset, outputs_numpy)
    ###I use this to extract exact_match value###
    eval_metric = metric.compute(predictions=prediction.predictions, references=prediction.label_ids)
